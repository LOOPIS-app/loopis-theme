<?php
/**
 * WPUM template for displaying the account forms.
 *
 * Modified by LOOPIS.
 */
...
<div class="wpum-template wpum-form wpum-account-form">
	...
	if ( 'success' === $updated ) :
		WPUM()->templates
			->set_template_data( array( 'message' => esc_html__( 'Din profil har uppdaterats.', 'wp-user-manager' ) ) )
	...
	if ( 'success' === $password_updated ) :
		WPUM()->templates
			->set_template_data( array( 'message' => esc_html__( 'Ditt lösenord har uppdaterats.', 'wp-user-manager' ) ) )
			->get_template_part( 'messages/general', 'success' );
	endif;
	?>
	...
	<?php do_action( 'wpum_before_account_form' ); ?>
		...
		<form action="<?php echo esc_url( $data->action ); ?>" method="post" id="wpum-submit-account-form" enctype="multipart/form-data">
			...
			<input type="submit" name="submit_account" class="button" value="<?php esc_html_e( 'Spara', 'wp-user-manager' ); ?>" />
