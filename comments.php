...
if (current_user_can('member') || current_user_can('administrator')) {
	include_once LOOPIS_THEME_DIR . '/assets/functions/user-extra/post-action-activity.php';
	include_once LOOPIS_THEME_DIR . '/assets/functions/user-extra/post-action-list.php';
	include_once LOOPIS_THEME_DIR . '/assets/functions/user-extra/post-action-comments.php';
} 
if ( current_user_can('loopis_storage_book') && in_category( 'storage' ) ) {
	include_once LOOPIS_THEME_DIR . '/assets/functions/admin-extra/storage-handling.php';
}
...
<?php if (in_category( 'first' )) : ?>
	...
	include_once LOOPIS_THEME_DIR . '/assets/output/post/gift/storage-book.php';
	endif; ?>
...
<?php if (in_category( 'paused' )) : ?>
	...
	<?php if ( $current == $author ) : ?>
		<p>Gå till dina <span class="big-link"><a href="/profil/posts/?status=paused">😎 Pausade annonser</a></span> för att aktivera.<p>
	<?php endif;?>
...
<?php if ( $current == $author ) : ?>
	<p>Gå till dina <span class="big-link"><a href="/profil/posts/?status=archived">⭕ Arkiverade annonser</a></span> för att aktivera.<p>
<?php endif;?>
...
<?php if (in_category( 'booked_locker' )) : ?>
	...
	<?php if ( $current == $fetcher ) : ?>
		<p>⏳ Du väntar på att <span class="link">👤<?php echo $authorlink;?></span> ska lämna i <span class="label"><i class="fas fa-walking"></i><?php echo $location; ?></span> <?php include LOOPIS_THEME_DIR . '/assets/output/post/gift/timer-locker.php';?>...</p>
	<?php endif;?>
	
	<?php if ( $current == $author ) : ?>
		<p><i class="fas fa-walking"></i>Dags att lämna i <span class="label">🌈 <?php echo $location; ?></span></p>
		<p><?php include LOOPIS_THEME_DIR . '/assets/output/post/gift/timer-locker.php';?></p>
...
<?php if (in_category( 'locker' )) : ?>
	...
	<?php if ( $current == $fetcher ) : ?>
		...
		<p><?php include LOOPIS_THEME_DIR . '/assets/output/post/gift/timer-fetch.php';?></p>
		<p>🔓 Kod till skåpet: <span class="code"><?php echo do_shortcode('[code_snippet id=93]');?></span></p>
		<?php if(isset($_POST['fetched'])) { action_fetched ($post_id); } ?>
...
<hr>
	<div id="commentlist-container" class="comment-tab">			
		...
		<?php if ( $comment_count == 0 ) {
			echo '<div class="comment-body"><p>💢 Ingen har ännu visat intresse.</p></div>';
			} else {
